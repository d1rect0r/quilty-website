 ...
 * sanitizer is the production defense; this assertion is the developer-
 * feedback layer that catches mistakes before they hit production
 * CloudWatch).
 *
  ...
  if (findings.length > 0) {
    throw new Error(
      `PHI keys detected in payload at ${context}: ${findings.join(', ')}. ` +
        `Either redact at the call site or pass through sanitize() from @quilty/security.`,
    );
  }
}
