...
M1.6 Workstreams C + D closed + Phase-B / Phase-C retroactive audit + doc-reconciliation pass landed. Foundation phase complete. Sprint metrics: 857+ tests across 17 workspaces, ≥98% type-coverage, 17/17 verify gates green, 3 new packages (`@quilty/search`, `@quilty/tokens`, `@quilty/workflow`). ADRs now run 0001-0030: the M1.6 batch added 0018 bundle budgets, 0019 search, 0020 design tokens, 0021 workflow engine, 0022 service worker, 0023 vaping-cessation regulatory classification, 0024 multi-state CHD posture, 0025 retention + research carve-out, 0026 pre-AI compliance shape; the subsequent hardening/auth pass added 0027 zero-PHI website runtime boundary, 0028 server-side ConsentState single source of truth, 0029 BFF auth architecture (token-handler flow), 0030 fail-closed configuration + adapter selection. All structural decisions D1-D175 + U1-U10 + D176-D178 (vaping-cessation framing reconciliation) locked — see `docs/website_strategy_discussion.md`.
...
- **Observability:** Sentry Business tier (errors + RUM + error-triggered replay) from day-one (D42a); **Amplitude all-in for web + mobile analytics** (D42b _revised 2026-05-19 — supersedes the Round-5 PostHog-for-web lock; see the ADR-0004 revision note_); session replay is **Sentry-only** (Amplitude Session Replay rejected, D68); OpenTelemetry is owned by the `@sentry/nextjs` v10 SDK (D56 _revised 2026-06-04 — `@sentry/nextjs` v10 is OTel-native and owns the OpenTelemetry provider; the `@vercel/otel` dependency + the `registerOTel()` call in `instrumentation.ts` were removed (two providers compete for the global context manager and break request isolation); Sentry now auto-consumes spans started via vendor-neutral `@opentelemetry/api`_)
...
- **Step-up auth** via `prompt=login` + server-side `elevated_until` flag (10-min fixed window, `max_age=600`) for sensitive actions (email/password change, account delete, payment method change, MFA management, data export) — D54 _Round-5; window revised 5 min → 10 min per ADR-0029 (5 min sat below every enterprise norm — GitHub 2h, Google Cloud 15m)_.
