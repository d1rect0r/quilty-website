...
2. **`website-baseline` Terraform** — author + apply: GitHub OIDC provider, `dev` + `preview` deploy roles + permission boundaries, WAF Web ACL, SSM params (WAF ARN, hosted-zone ID), Secrets Manager pseudonym pepper, **plus** canaries/alarms/SNS/budgets/log-buckets/GuardDuty. _(quilty-aws; the primary blocker.)_ **Open question to resolve first:** CloudFront flat-rate plan vs standalone WAF ACL (§10).
3. **Website prep** — create Sentry project + DSN (deploy-gate var); set GitHub `production` environment protection (required reviewer); close the cheap `sst.config.ts` gaps (image-opt memory, `warm:1`, price class, TLS min, invalidation); enable GHAS secret scanning. _(Note: `/api/health`, the `sst-deploy.md`/`log-retention.md` runbooks, and the `deploy.yml` pepper-env wiring already exist — see §12 audit corrections.)_
4. **First `sst deploy --stage dev`** — provisions CloudFront + SSR Lambda + S3 + ACM cert request (SST creates the `.com` cert from scratch). Cert blocks on DNS validation (expected). Capture CloudFront domain + ACM validation CNAMEs.
5. **Pattern A DNS PR** (`quilty-aws/dns/`) — write ACM validation CNAMEs + apex/www alias (A + AAAA) → cert validates.
6. **Re-deploy** — SST attaches the issued cert. Apply sitewide `noindex`. Verify canaries green, alarms armed, WAF blocking. **Site is live + stable + hidden-from-search.**
...
| E11 | ACM cert `my-quilty.com` + `*.my-quilty.com` (us-east-1) | DNS-validated; CloudFront requires us-east-1. **No `.com` cert exists today** (only `.app`); SST creates it on first deploy                                                              | one-time(auto-renew) | ⛔ SST creates on first deploy → needs DNS validation                                                                          |
| E12 | TLS min protocol `TLSv1.2_2021`                          | CloudFront viewer policy (upgrade to `TLSv1.2_2025` in Track 3)                                                                                                                          | one-time             | 🔧 gap in `sst.config.ts`                                                                                                      |
...
| E19 | Cache invalidation (basic `/*`)                          | On deploy; OpenNext handles hashed assets                                                                                                                                                | per-deploy           | 🔧 not yet explicit in config                                                                                                  |
| E20 | Price class `PriceClass_100`                             | Cost control (US/EU edges). _Becomes free `PriceClass_All` if flat-rate plan adopted — see §10_                                                                                          | one-time             | 🔧 gap — not set                                                                                                               |
| E21 | IPv6 + AAAA alias                                        | Enable on distribution + AAAA record                                                                                                                                                     | one-time             | 🔧 AAAA record needed                                                                                                          |
| E22 | CFF `security-headers`                                   | HSTS/XCTO/XFO/COOP/CORP injection. _Track 2 (E2-10): migrate static headers to native Response Headers Policy_                                                                           | ongoing              | ✅ done                                                                                                                        |
| E23 | CFF `gpc-force-off`                                      | `Sec-GPC:1` → default-deny consent cookie                                                                                                                                                | ongoing              | ✅ done · ⚠ `functional` parity bug (CFF=`true` vs canonical `false`; the parity test also asserts the wrong value — fix both) |
| E24 | CFF `robots-tag-defense`                                 | `X-Robots-Tag: noindex` on `/api`,`/auth`,`/account`,`/dev`,status                                                                                                                       | ongoing              | ✅ done                                                                                                                        |
| E25 | WAF Web ACL (CLOUDFRONT)                                 | Common + KnownBadInputs + IpReputation + **Bot Control (Common)** + **Host-header allowlist** (`my-quilty.com`/`www`) + rate-limit; ATP deferred to auth milestone; **hard deploy gate** | ongoing              | ⛔ ACL in `website-baseline/` (not started)                                                                                    |
...
| E30 | Sitewide `noindex` (placeholder phase)                   | Meta + header until launch; drop at go-public                                                                                                                                            | toggle               | ➕ to add                                                                                                                      |
...
| C2  | Image-optimization Lambda memory           | Explicit `imageOptimization: { memory: '1536 MB' }` (Sharp)                                                                                                                                                      | one-time | 🔧 gap — add                  |
| C3  | Revalidation Lambda + SQS FIFO             | OpenNext auto-provisions                                                                                                                                                                                         | auto     | ✅ auto (DLQ → Track 2)       |
| C4  | Warmer Lambda                              | Add `warm: 1` in `sst.config.ts` (SST API; not `open-next.config.ts`). _Aug-2025 INIT billing makes cold starts a cost item; if cold-start rate >5% post-launch, upgrade to provisioned concurrency 2 (Track 2)_ | one-time | 🔧 gap — add                  |
...
| B2-6 | Runtime Secrets Manager fetch for pepper    | Removes redeploy-on-rotation                                                                           | ➕     |
...
- **CloudFront flat-rate pricing plan (Nov 2025)** — evaluate Pro (~$15/mo bundles CloudFront + WAF + Route 53 + edge compute; blocked-request traffic doesn't count against allowance) vs standalone WAF ACL **before authoring `website-baseline/`**. ⚠ Flat-rate plans do **not** support Route 53 Accelerated Recovery (O7) and collapse the `PriceClass_100`/`All` distinction (E20). Resolve first.
- **CFF vs Response Headers Policy** — adopt native RHP for static headers (E2-10) and keep CFF for dynamic logic only; reduces the maintenance surface that produced the E23 bug.
- **OU/SCP final shape** — Workloads/Production vs a dedicated Customer-Surface OU (both defensible; Non-Prod ruled out).
- **`shared-services` account + Vault Lock mode** — GOVERNANCE vs COMPLIANCE (confirm with auditor); Track 2 (cross-repo, `quilty-aws`).
- **GPC `functional` parity bug** (E23) — reconcile CFF (`true`) vs canonical taxonomy/proxy.ts (`false`); the parity test asserts the wrong value, so CI does not catch the divergence. Fix code **and** test before relying on the edge cookie.
...
- E23 parity bug understated → the parity **test** also asserts the wrong value; both need fixing.
