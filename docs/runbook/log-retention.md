...
- hashed actor IDs — produced by the `@quilty/security` sanitizer's `hashId()`. Emits `hmac.v1:<24-hex>` when `QUILTY_PSEUDONYM_PEPPER` is present (production deploy-gate vends the pepper from AWS Secrets Manager — never SSM Parameter Store, never config-bundled, per EDPB 01/2025); emits `dev:<24-hex>` (plain SHA-256) when the env var is unset (dev / test paths). 96-bit prefix per NIST SP 800-107r1 §5.1 collision floor (was 64-bit, replaced — the prior prefix collapsed collision resistance to ~2^32 at log scale). Versioned namespace (`hmac.v1:`) lets the audit channel distinguish pre-rotation vs post-rotation pseudonyms without re-hashing history. Annual pepper rotation per stage; bump the version suffix on rotation. Browser-runtime client bundles always land on the `dev:` namespace because `QUILTY_PSEUDONYM_PEPPER` is intentionally NOT a `NEXT_PUBLIC_*` var — client-side pseudonymization adds zero protection against client adversaries; the server-side HMAC is what defends against log-side adversaries (CloudWatch read access vs Cognito user-pool mapping).
