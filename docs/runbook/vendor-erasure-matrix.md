...
| Field                          | Value                                                                                                                                                                                                                                                                                                                       |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Erasure API endpoint           | PostHog Data Deletion API: `POST https://us.posthog.com/api/projects/{project_id}/persons/{person_id}/delete_events/` + `DELETE https://us.posthog.com/api/projects/{project_id}/persons/{person_id}/`                                                                                                                      |
| Auth method                    | Personal API key with project-admin scope (rotated quarterly; stored in `quilty-1password`)                                                                                                                                                                                                                                 |
| Parameters                     | `person_id` resolved from `distinct_id` set to `quilty_sub` at session start                                                                                                                                                                                                                                                |
| Fields deleted                 | All events keyed on `person_id` + the person record itself; session replay blobs purged from blob storage within PostHog's GDPR-deletion SLA                                                                                                                                                                                |
| Fields retained (Art 17(3)(b)) | Aggregate cohort + funnel counters (non-personal, PostHog-owned analytics on its own infra)                                                                                                                                                                                                                                 |
| BAA status                     | REQUIRED-BEFORE-USE — PostHog Cloud Boost offers BAA on paid tier; D165 explicit-request verification mandatory before consent flips off default-deny                                                                                                                                                                       |
| Cross-effects                  | Replay-blob purge is asynchronous; the orchestrator polls the deletion-status endpoint and waits for completion before closing the DSAR ticket. PostHog also recommends fingerprint-cookie + localStorage scrub at the same time — the orchestrator emits a `posthog.reset()` signal via the analytics adapter when present |
