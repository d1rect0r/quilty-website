...
The analytics vendor for both the website tier and the mobile tier is Amplitude (D42b, revised 2026-05-19). Pre-launch posture starts without BAA per the locked policy; upgrade to Amplitude Enterprise BAA pre-launch when PHI risk becomes real. Replay stays Sentry-only (error-triggered, mask-all by default) per D68 — Amplitude Session Replay is explicitly rejected for its HTML-attribute leak on `alt`/`title`/`placeholder`/`aria-label`/`value`/`data-*`. (See ADR-0010 + the `@quilty/observability` port for the role-shaped vendor-agnostic API.)
...
| Amplitude              | Web + mobile analytics, feature flags, experiments | REQUIRED-BEFORE-USE | D42b (revised 2026-05-19). Pre-launch posture starts without BAA per the locked policy; Enterprise BAA upgrade pre-launch. Default-deny consent gate enforced until BAA + consent are both green. Session Replay product is rejected (D68) |
