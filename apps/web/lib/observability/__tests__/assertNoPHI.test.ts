...
describe('assertNoPHI', () => {
  ...
  it('throws in development on PHI-shaped keys', () => {
    ...
    expect(() =>
      assertNoPHI({ user_id: 'x', email: 'user@example.com' }, 'test'),
    ).toThrow(/PHI keys detected/);
  });
  ...
  it('throws when PHI is nested deep', () => {
    ...
    expect(() =>
      assertNoPHI(
        { ctx: { profile: { diagnosis: 'gad' } } },
        'analytics:profile_loaded',
      ),
    ).toThrow(/diagnosis/);
  });
  ...
  it('throws on PHI inside arrays', () => {
    vi.stubEnv('NODE_ENV', 'development');
    expect(() =>
      assertNoPHI({ items: [{ ok: true }, { medication: 'x' }] }, 'test'),
    ).toThrow(/medication/);
  });
  ...
  it('does not throw on payloads with only safe keys', () => {
    ...
    expect(() =>
      assertNoPHI(
        { route: '/en/account', method: 'GET', duration_ms: 142 },
        'test',
      ),
    ).not.toThrow();
  });
  ...
  it('is silent in production (sanitize() is the production defense)', () => {
    ...
    expect(() =>
      assertNoPHI({ email: 'user@example.com' }, 'test'),
    ).not.toThrow();
  });
